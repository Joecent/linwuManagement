	...
	<div class="member_page">
				...
				<el-tab-pane label="审批成功" name="first" class="tabpane">
						<table style="width:100%" class="table">
							...
							<tbody class="tabelbody">
								<tr v-for="(item,index) in staff.tableData" :key='index'><!--<a :href="item.code" download='QrCode'>-->
									<td><span class="mobile">姓名:</span><span class='value_right'>{{item.username}}</span></td>
									<td><span class="mobile">手机号:</span><span class='value_right'>{{item.phone}}</span></td>
									<td><span class="mobile">待成团:</span><span class='value_right'>{{item.waitGroup}}</span></td>
									<td><span class="mobile">余额:</span><span class='value_right'>￥{{Number(item.group_money).toFixed(2)}}</span></td>
									<td class='td min_width max_width'><span class="mobile">申请日期:</span><span class='value_right'>{{item.create_time*1000 | dataFilter(1)}}</span></td>
									<td><span class="mobile">来源二维码:</span><img class='value_right imgUrl' @click="downloadFile(item.user_id)" :src="imgUlr"></td>
									<td class="last_btn">
									...
									<td class="last_btn">
										<div class="data-info" @click="handleSearch(item.user_id,item.username,item.waitGroup)">查看个人资料</div>
									</td>
								</tr>
							</tbody>
						</table>
		...
		<!--待审批人员拒绝-->
			...
			<el-dialog title="设置排序" :visible.sync="sortalert" width="30%" :modal-append-to-body='false'>
				<div class="head"><span class="infoname">团长排序:</span><el-input v-model="sortValue" class="txt"></el-input></div> 
				<span slot="footer" class="dialog-footer">
...
import {get_saleUserAdmin,get_requestStatus,downfile,get_groupApplyAdmin,up_groupApplyAdmin,downloadGroupQRcode,getGroupSort,upGroupSort} from '@/utils/api'
...
import pic from '@/assets/images/downsimg.png'
	export default{
		...
		methods:{
			...
			handleSearch(id,name,waitGroup){//查看详情
				this.$router.push({path:'/business/group/details',query:{id:id,name:name,waitGroup:waitGroup}})															
			},
			 handlepayment(e) {
			...
			downloadFile(id){
				...
				location="http://116.62.187.206/api/downloadGroupQRcode?shop_id=144&user_id="+id
			},
			handleTable2(page,pno){
				get_groupApplyAdmin({//审批成功
					shop_id:store.get('currentshopid'),
					limit:page,
					page:pno,
					verify_status:'1'
