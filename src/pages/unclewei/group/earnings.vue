<template>
	<div class="withdrawal">
						...
						<ul class="tabelbody">
							<li v-for="(item,index) in accept.tabel" :key="index">
								...
								<div class="paymentInfo">
									<span class="paymentInfo-px">支付方式：支付宝提现</span>
									<span>付款手机号：{{item.zfb_username}}</span>
									<span>付款姓名：{{item.zfb_name}}</span>
								</div>
		...
		<!--待审批人员拒绝-->
		<el-dialog title="确认拒绝" :visible.sync="DialogVisible" width="30%" center :modal-append-to-body='false'>
...
import {up_groupCashStatus,groupCashAdminDetail,getFriendDetail,upFriendStatus} from "@/utils/api";
      ...
      getFriendDetail({
		...
		updateTable3(limit, page, startTime, endTime){
				getFriendDetail({
					...
					console.log(res)
						if (res.status == "1") {
								this.notPayment.tabel= res.data
						}
				})
		},
	dateTime(value,type){
