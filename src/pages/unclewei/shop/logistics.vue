<template>
	<div class="logistics_page">
		<el-tabs v-model="activeName" @tab-click="handleClick">
			...
			<el-tab-pane label="达达配送" name="first">
				<div class="l-container-open">
					<div class="l-container-left">是否开启达达配送：</div>
				...
				<div class="l-container">
					<div class="l-container-left">商户编号：</div>
					<el-input :disabled="this.form2.edit" class="l-container-right" v-model="form2.source_id"></el-input>
					
				</div>
				<div class="l-container">
					<div class="l-container-left">门店编号：</div>
					<el-input :disabled="this.form2.edit" class="l-container-right" v-model="form2.dada_shopid"></el-input>
				</div>
				<div class="l-container">
				...
				<div class="l-container">
					<div class="l-container-left">联系人姓名：</div>
				...
				<div class="l-container">
					...
					<div class="l-container-select">
						<el-select v-model="form2.cityone" placeholder="请选择" @change="city_change" >
							<el-option v-for="item in form2.citys" :label="item.cityName" :value="item.cityCode"></el-option>
						</el-select>
					</div>
				</div>
				<div class="l-container">
					<div class="l-container-left">所在区/县：</div>
					<div class="l-container-select">
  						<el-input v-model="form2.citytwo" placeholder="请输入区/县"/>
					</div>
				</div>
				<div class="l-container">
...
import {get_province,get_citycode_list,get_city,get_county,get_dispatch,up_dispatch,get_showArea,add_expressFee,del_express,addCity,add_dada_shop,get_shop_dada_config,edit_data_shop,recharge,select_one} from '@/utils/api'
...
import zfb from "@/assets/images/zfb.png";
	export default{
		...
		methods:{
			//选择城市
			city_change(){
				for (var i = this.form2.citys.length - 1; i >= 0; i--) {
					if(this.form2.citys[i].cityCode==this.form2.cityone){
						this.form2.cityName=this.form2.citys[i].cityName
					}
				};
			},
			handleOpen1(e){
			...
			get_dada(){
				get_shop_dada_config({//请求达达页面数据
					...
					if(res.status=='1'){
						this.form2.dada_shopid = res.data.origin_shopid
						this.form2.source_id = res.data.source_id
				...
				if(this.form2.shop_name==''){
					...
					if(this.form2.edit){//判断是编辑还是新增
						edit_data_shop({
							...
							//origin_shopid:this.form2.dada_shopid,//门店编号
							city_name:this.form2.cityName,
							area_name:this.form2.citytwo,
							is_show:this.form2.is_show?'1':'0',
							receiverAddress:this.form2.address,//详细地址
							receiverLat:this.form2.latitude,//纬度
							receiverLng:this.form2.longitude,//经度
						...
						add_dada_shop({
							...
							receiverName:this.form2.user_name,
							receiverPhone:this.form2.shop_phone,//电话
							city_code:this.form2.cityone,//所选城市
							origin_shopid:this.form2.dada_shopid,//门店编号
							source_id:this.form2.source_id,
							city_name:this.form2.cityName,
							area_name:this.form2.citytwo,
							is_show:this.form2.is_show?1:0,
							receiverAddress:this.form2.address,//详细地址
			...
			handleprovinceList(e){
				...
				for (var i = this.form2.citys.length - 1; i >= 0; i--) {
					if(this.form2.citys[i].cityCode=e){
						this.form2.city_name=this.form2.citys[i].cityName
					}
				
				};
				// conif(sole.log(e.citys[i].cityCode=e){
					//this.form2.cityName=this.form2.citys[i].cityName
		...
		created(){
			this.get_show()
			this.get_dada()
			get_citycode_list({}).then(res=>{//请求省份

				this.form2.citys=res.data
				//console.log(res.error_code,'tttttt')
			})
