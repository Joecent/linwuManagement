...
<div class="goodspage">
		...
		<div v-for="(item,index) in goodsinfo" class="goods_info">
			...
			<div class="w-block">
            	 <el-pagination @current-change="handleCurrentChange" :page-size="10" layout="prev, pager, next, jumper" :page-count="pagecount" :pager-count="5" :current-page.sync="currentPage"></el-pagination>
       		 </div>
		</div>
		<!-- 删除商品 -->
		...
		<!--排序-->
		<el-dialog title="取消推荐" class="remove" :visible.sync="show_editgoods_sort" :modal-append-to-body='false'>
			<el-input v-model="adjustorder_new" placeholder="请输入新的排序"/>
			<div class="question">
				<button class="cancle" @click="show_editgoods_sort=false">取消</button>
				<button class="comfirm" @click="adjust_comfirm()">确认</button>
			</div>
        </el-dialog>
	</div>
</template>
...
import {sel_cate,goods_show,goods_del,admin_search_goods,set_on_sale,admin_goods_list,set_commend,adjust_order} from '@/utils/api'
	export default{
		...
		methods:{
			...
			search(){
				this.currentPage=1
				admin_search_goods({goods_name:this.search_input,shop_id:store.get('currentshopid'),status:1}).then((res)=>{
			...
			to_editgoods_sort(id,sort){
				this.goods_id=id
				this.adjustorder_new=sort
				this.show_editgoods_sort=true	
			},
			adjust_comfirm(){
				//this.$message.error('请输入商品序号')
				if(this.adjustorder_new==''){
					this.$message.error('请输入商品序号')
				}else{adjust_order({adjustorder:this.adjustorder_new,goods_id:this.goods_id}).then((res)=>{
					if(res.error_code==1003){
						this.$message.error(res.msg)
					};
					if(res.error_code==1000){
			...
			toeditgoodsinfo(id,index){
				this.$router.push({path:'/unclewei/goods/goods_edit',query:{goods_id:id,currentPage:this.currentPage}})
			}
		},
		created(){
