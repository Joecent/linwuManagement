	...
	<div class="cates_page">
		...
		<div class="cates">
			<div class="cate" v-for="item in cates">
				...
				<div class="editcates">
					<div class="edit data_btn btn_success" @click="toeditcate(item.cate_id,item.cate_name,item.sort,item.cate_img)">编 辑</div>
					<div class="delete">
...
import { sel_cate, del_cate, add_cate, edit_cate,goodsGroups,upGoodsGroupStatus} from "@/utils/api";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    showcate() {
      goodsGroups({ shop_id: store.get("currentshopid") }).then(res => {
				console.log(res)
        this.cates = res.data;
      });
    },
