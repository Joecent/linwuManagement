    		...
    		<div class="order_information">
				...
				<div class="goodsimg" v-for='items in item.goodsinfo'>
					...
					<div class="goodsinfo">
					 	...
					 	<div class="timeprice">
					 		<div class="pay"><div class="pay_money">￥{{items.goodscost}}</div><div class="pay_sum">/ <span style="font-size:14px">数量</span>：{{items.goodsnum}}</div></div>
					 	</div>
					</div>
				</div>
...
import {admin_wait,send,admin_waits} from '@/utils/api' 
	export default{
		data(){
			return{
				pagecount:1,
		...
		admin_wait(){
			admin_waits({shop_id:store.get('currentshopid'),page:this.currentPage}).then((res)=>{
				...
				if (res.error_code==1000) {
					...
					console.log(this.order.length,'222')
					for (var i=0;i<this.order.length;i++) {
						for (var p=0;p<this.order[i].goodsid.length;p++) {
							console.log('111')
			...
			// admin_wait({shop_id:store.get('currentshopid'),page:this.currentPage}).then((res)=>{			
