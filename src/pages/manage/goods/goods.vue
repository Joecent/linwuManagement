<template>
	<div class="goodspage">
		...
		<!-- 删除商品 -->
		 <el-dialog title="下架商品" class="remove" :visible.sync="is_delete">
		...
		<!--排序-->
		<el-dialog title="设置排序" class="remove" :visible.sync="show_editgoods_sort">
...
import {sel_cate,goods_show,goods_del,admin_search_goods,set_on_sale,admin_goods_list,set_commend,adjust_order} from '@/utils/api'
	export default{
		...
		methods:{
			...
			search(){
				this.currentPage=1
				admin_search_goods({goods_name:this.search_input,page:this.currentPage,shop_id:store.get('currentshopid')}).then((res)=>{
			...
			showaddgoodsmethod(){
				this.$router.push('/home/goods/goods_add')
			},
