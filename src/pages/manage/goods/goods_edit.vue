 ...
 "justifyleft", "justifycenter", "justifyright", "justifyfull", "insertorderedlist", "insertunorderedlist", "subscript", "superscript", "|", "/", "formatblock", "fontname", "fontsize", "|", "forecolor", "hilitecolor", "bold", "italic", "underline", "strikethrough",  "|", "table", "hr","image"],
		...
		methods:{
			...
			change_index(index,indexs){
				...
				console.log(this.spec_index,this.spec_value_index,'777')
			},
	...
	//分类切换调用
      ...
      createTableData() {
        if (this.toggle_specs === 0) {
          ...
          this.table_data = obj
          console.log(this.table_data,'222')
        }
        if (this.toggle_specs === 1) {
      ...
      afterChange() {
        ...
        console.log(this.goodsinfo.goods_desc,'1111')
      },
      afterBlur() {
      },
      onContentChange() {
      ...
      get_goods_info(id){
      	get_goods({goods_id:id}).then((res)=>{
      		console.log(res)
				 ...
				 //this.cate_change(res.data.goods[0].cate_id)
				 this.editorText=res.data.goods[0].goods_desc
				 this.goodsinfo.link=res.data.goods[0].goods_source
				 this.goodsinfo.code=res.data.goods[0].qrcode
				 this.goodsinfo.group_num=res.data.goods[0].group_num
				 this.images_list.push({name:'image',url:res.data.goods[0].goods_img})
				 this.goodsinfo.goods_img.push(res.data.goods[0].goods_img)
				 	...
				 	search_spec({cate_id: this.cate_id, shop_id: store.get('currentshopid')}).then((res) => {
          			if (res.error_code == 1000) {
            			...
            			this.specs = []
        				 ...
        				 var value_pro=[]
				 	 	for (var i = item.key_names.length - 1; i >= 0; i--) {
				 			value_pro.push({
				 		...
				 		console.log(value_pro,'777')
        				this.use_specs=[{name:parseInt(item.keys[0]),specs_item:this.specs[0].item,value:value_pro}]
        				this.table_data=this.use_specs[0]
				 		console.log(this.table_data,'7777')
        			})
				 ...
				 //this.use_specs=[{name:'数学',value:[3]}]
      	})
      }
			...
			// 	}else if (this.goodsinfo.goods_img.length==0) {
			// 		this.$message.error('请至少添加一张商品图片')
			// 	}else if (this.goodsinfo.price=='') {
			// 		this.$message.error('请输入商品价格')
			// 	}else if (this.goodsinfo.score=='') {
			...
			// 		goods_img:this.goodsinfo.goods_img[0],
			...
			// 		cate_id:this.goodsinfo.cate, 
			// 		shop_id:store.get('currentshopid'),
			// 		photo:this.goodsinfo.goods_img.slice(1),
			// 		single_p_profit:this.goodsinfo.single_p_profit,
			// 		group_p_profit:this.goodsinfo.group_p_profit,
			// 		p_price:this.goodsinfo.p_price,
			// 		real_goods_score:this.goodsinfo.real_goods_score,
			// 		goods_score_expect:this.goodsinfo.goods_score_expect,
			// 		goods_source:this.goodsinfo.goods_source
			// 	}).then((res)=>{
			// 		if (res.error_code==1000) {
			// 			this.$message.success('添加成功')
			...
			// 		if (res.error_code==1000) {
			// 			this.$message.success('修改成功')
			// 			if (this.goodsinfo.is_on_sale==1) {
			// 			this.$router.push('/home/goods/goods')}
			// 			if (this.goodsinfo.is_on_sale==0) {
			// 				this.$router.push('/home/goods/goods_off')
			// 			};
			// 		}else if (res.error_code==1003) {
			// 			this.$message.error(res.msg)
			// 		};
