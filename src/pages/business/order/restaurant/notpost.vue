...
<div class="orders">
    <div class="order" v-for='item in order'>
    		...
    		<div class="order_information">
				...
				<div class="postbutton" @click="tosendgoods(item.order_id)">出餐</div>
	  		</div>	
    </div>
    <div class="block">
            	 <el-pagination
    ...
    <el-dialog title="是否确认出餐" :visible.sync="tosend" label-width="80px" style="width:100%;margin-top:100px">
...
import {admin_wait,send,admin_waits,admin_all_wait} from '@/utils/api' 
	export default{
		...
		admin_wait(){
			admin_waits({shop_id:store.get('currentshopid'),page:this.currentPage,order_type:2}).then((res)=>{
				var order_goods=[]
		...
		onSubmit(order_id){
			send({order_id:order_id,shipping_name:'',shipping_code:''}).then((res)=>{
				this.admin_wait()
				})
			this.tosend=false
		},
