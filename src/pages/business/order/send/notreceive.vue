...
<div>
		...
		<div class="orders">
			<div class="order" v-for='(item,index) in order'>
				...
				<div class="shipping" >
					<div class="shipping_name" v-if="send_type==1">配送方式：自主配送</div>
					<div class="shipping_name" v-if="send_type==2">配送方式：美团配送</div>
					<div class="showshipping" @click="showexpress(item.order_id)" v-if="send_type==2">配送状态</div>
				</div>
				<div class="status">等待确认送达</div>
				</div>
			</div>
			<div class="block">
            	 <el-pagination
...
import {admin_already,get_shipping} from '@/utils/api' 
	export default{
		...
		admin_already(){
			admin_already({shop_id:store.get('currentshopid'),order_type:1,page:this.currentPage}).then((res)=>{
				var order_goods=[]
