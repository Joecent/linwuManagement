...
import {admin_wait,send,admin_waits,admin_all_wait,dada_index,get_dada_status} from '@/utils/api' 
	export default{
		data(){
			return{
		...
		admin_wait(){
			admin_waits({shop_id:store.get('currentshopid'),page:this.currentPage,order_type:1}).then((res)=>{
				var order_goods=[]
