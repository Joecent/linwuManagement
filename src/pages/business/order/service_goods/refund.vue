...
import {get_refund,solve} from '@/utils/api' 
	export default{
		...
		get_refund(){
			get_refund({shop_id:store.get('currentshopid'),order_type:0}).then((res)=>{
				var order_goods=[]
				if (res.error_code==1000) {
					...
					for (var i = this.order.length - 1; i >= 0; i--) {
						...
						order_goods.push({goodscost:this.order[i].goods_price[p],
										  goodsname:this.order[i].goods_name[p],
										  goodsnum:this.order[i].goods_num[p],
