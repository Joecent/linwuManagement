<template>
  <div class="coupon_page">
    <div class="lee-title1 w-scan">
      <span>线下核销优惠券</span>
      <span @click="sys_click()" class="scan">扫一扫</span>
    </div>
    ...
    <div class="addcoupon">
      <div class="addcoupon_button" @click="showaddcoupon=true">
        <i class="el-icon-plus" style="margin-right:5px"></i>新增优惠券
      </div>
      ...
      <div class="coupon_frame" v-for="item in coupons">
        <div class="coupon_info">
          ...
          <div class="money_info">
            <div class="price_money" v-if="item.cash_type==0">
              ￥
              <span class="cash_money">{{item.cash_money}}</span>
            </div>
            <div class="type" v-if="item.cash_type==0">抵用券</div>
            <div class="price" v-if="item.cash_type==1">
              <span class="cash_discount">{{item.cash_money}}</span>折
            </div>
            <div class="type" v-if="item.cash_type==1">折扣券</div>
          </div>
          <div class="condition_info">
            <div
              class="money_condition"
              v-if="item.cash_type==0"
            >满{{item.pay_money}}减{{item.cash_money}}</div>
            <div
              class="money_condition"
              v-if="item.cash_type==1"
            >满{{item.pay_money}}享{{item.cash_money}}折优惠</div>
            <div class="range" v-if="item.cash_state==2">新人领取</div>
            <div class="range" v-if="item.cash_state==0">店内领取</div>
    ...
    <el-dialog title="新增优惠券" :visible.sync="showaddcoupon" :modal-append-to-body="false">
      <el-form :model="coupon">
        ...
        <el-form-item label="使用场景" :label-width="formLabelWidth">
          <el-select
            placeholder="请选择优惠券类型"
            v-model="coupon.cash_state"
            style="width:80%"
            size="medium"
          >
            <el-option label="新人领取" :value="2"></el-option>
            <el-option label="店铺领取" :value="0"></el-option>
        ...
        <el-form-item label="优惠券额度" :label-width="formLabelWidth">
          <el-input
            placeholder="请输入优惠金额"
            v-if="this.coupon.cash_type==0"
            style="width:80%"
            size="medium"
            v-model="coupon.cash_money"
          ></el-input>
          <el-input
            placeholder="请输入优惠折扣"
            v-if="this.coupon.cash_type==1"
            style="width:80%"
            size="medium"
            v-model="coupon.cash_money"
          ></el-input>
        ...
        <el-form-item label="满足消费金额" :label-width="formLabelWidth">
          <el-input
            placeholder="请输入使用最低消费金额"
            size="medium"
            style="width:80%"
            v-model="coupon.pay_money"
          ></el-input>
        </el-form-item>
        ...
        wx.scanQRCode({
          ...
          success: function(res) {
            ...
            window.location.href =
              "https://www.linwushop.com/business/marketing/coupon?code=" +
              result;
          }
        });
      });
